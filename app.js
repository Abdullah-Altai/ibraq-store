...
function renderProducts(){
 ...
 $("#productsGrid").innerHTML=list.map((p,i)=>`<article class="product-card" style="animation-delay:${i*.05}s">
   <div class="product-image-wrap">
    <img class="product-image" data-open="${p.id}" id="main-${p.id}" src="${p.images?.[0]||""}" alt="${loc(p,"name")}">
    ${loc(p,"badge")?`<span class="badge">${loc(p,"badge")}</span>`:""}
    ${p.images?.length>1?`<div class="thumbs">${p.images.slice(0,4).map((im,x)=>`<img class="thumb ${x===0?"active":""}" data-product="${p.id}" data-index="${x}" src="${im}">`).join("")}</div>`:""}
   </div>
...
function renderAds(){
 ...
 $("#adsSlider").innerHTML=ads.map(a=>`<div class="ad-card" data-ad="${a.id}"><img src="${a.image}"><div class="ad-overlay"><h3>${loc(a,"title")}</h3><p>${loc(a,"subtitle")}</p></div></div>`).join("");
...
function openQty(id){selectedProduct=db.products.find(p=>p.id===id);if(!selectedProduct)return;$("#qtyImage").src=selectedProduct.images[0];$("#qtyTitle").textContent=loc(selectedProduct,"name");$("#qtyInput").value=1;$("#qtyInput").removeAttribute("max");$("#qtyModal").classList.remove("hidden");setTimeout(()=>$("#qtyInput").select(),60)}
let addBusy=false;
...
$("#clearCart").onclick=()=>{cart=[];saveCart(cart);renderCart()};$("#qtyMinus").onclick=()=>$("#qtyInput").value=Math.max(1,+$("#qtyInput").value-1);$("#qtyPlus").onclick=()=>$("#qtyInput").value=Math.max(1,+$("#qtyInput").value+1);$("#confirmAdd").onclick=addSelected;
